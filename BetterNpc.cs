...
namespace Oxide.Plugins
{
    ...
    internal class BetterNpc : RustPlugin
    {
        ...
        private void UpdateCustomDataFiles()
        {
            foreach (KeyValuePair<string, CustomSpawnPoint> dic in _customSpawnPoints)
            {
                if (dic.Value == null) continue;
                if (dic.Value.ID == null) dic.Value.ID = string.Empty;
                if (dic.Value.Presets == null) dic.Value.Presets = new List<PresetConfig>();
                foreach (PresetConfig preset in dic.Value.Presets.Where(p => p != null))
                {
                    if (preset.Config == null) preset.Config = new NpcConfig { BeltItems = new List<NpcBelt>(), WearItems = new List<NpcWear>(), Kits = new List<string>() };
                    if (preset.Config.BeltItems == null) preset.Config.BeltItems = new List<NpcBelt>();
                    foreach (NpcBelt belt in preset.Config.BeltItems.Where(b => b != null)) if (belt.Ammo == null) belt.Ammo = string.Empty;
                    if (preset.PrefabLootTable == null) preset.PrefabLootTable = new PrefabLootTableConfig { Prefabs = new List<PrefabConfig>() };
                    if (preset.PrefabLootTable.Prefabs == null) preset.PrefabLootTable.Prefabs = new List<PrefabConfig>();
                    if ((preset.TypeLootTable == 4 || preset.TypeLootTable == 5) && preset.PrefabLootTable.Prefabs.Count == 0)
                        preset.PrefabLootTable.Prefabs.Add(new PrefabConfig { Chance = 100f, PrefabDefinition = "assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_oilrig.prefab" });
                }
                Interface.Oxide.DataFileSystem.WriteObject($"BetterNpc/Custom/{dic.Key}", dic.Value);
            }
        }
        ...
        {
            string monName = monument?.name ?? string.Empty;
            string displayName = monument?.displayPhrase?.english != null ? monument.displayPhrase.english.Replace("\n", string.Empty) : monName;
            if (string.IsNullOrEmpty(displayName)) return string.Empty;
            if (monName.Contains("harbor_1")) return "Small " + displayName;
            if (monName.Contains("harbor_2")) return "Large " + displayName;
            if (monName.Contains("desert_military_base_a")) return displayName + " A";
            if (monName.Contains("desert_military_base_b")) return displayName + " B";
            if (monName.Contains("desert_military_base_c")) return displayName + " C";
            if (monName.Contains("desert_military_base_d")) return displayName + " D";
            return displayName;
        }
        ...
        private void LoadMonumentSpawnPoints()
        {
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("BetterNpc/Monument/"))
            {
                ...
                if (spawnPoint != null)
                {
                    ...
                    _monumentSpawnPoints[fileName] = spawnPoint;
                }
        ...
        private void LoadUnderwaterLabSpawnPoints()
        {
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("BetterNpc/Monument/Underwater Lab/"))
            {
                ...
                {
                    if (!_config.EnabledMinLogs) Puts($"File {fileName} has been loaded successfully!");
                    _underwaterLabSpawnPoints[fileName] = spawnPoint;
                }
                else PrintError($"File {fileName} is corrupted and cannot be loaded!");
            }
        }
        ...
        {
            if (TerrainMeta.Path == null || TerrainMeta.Path.DungeonBaseEntrances == null) return;
            foreach (DungeonBaseInfo baseModule in TerrainMeta.Path.DungeonBaseEntrances)
            {
                ...
                if (baseModule.Links == null) continue;
        ...
        private void LoadTunnelSpawnPoints()
        {
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("BetterNpc/Monument/Tunnel/"))
            {
                ...
                {
                    if (!_config.EnabledMinLogs) Puts($"File {fileName} has been loaded successfully!");
                    _tunnelSpawnPoints[fileName] = spawnPoint;
                }
                else PrintError($"File {fileName} is corrupted and cannot be loaded!");
            }
        }
        ...
        {
            if (TerrainMeta.Path == null || TerrainMeta.Path.DungeonGridCells == null) return;
            foreach (DungeonGridCell gridCell in TerrainMeta.Path.DungeonGridCells)
            {
                if (gridCell == null || string.IsNullOrEmpty(gridCell.name)) continue;
        ...
        private void LoadCustomSpawnPoints()
        {
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("BetterNpc/Custom/"))
            {
                ...
                if (!_config.EnabledMinLogs) Puts($"File {fileName} has been loaded successfully!");
                _customSpawnPoints[fileName] = spawnPoint;
            }
        }
        ...
        private void LoadEventSpawnPoints()
        {
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("BetterNpc/Event/"))
            {
                ...
                else
                {
                    ...
                    {
                        if (!_config.EnabledMinLogs) Puts($"File {fileName} has been loaded successfully!");
                        _eventSpawnPoints[fileName] = spawnPoint;
                    }
                    else PrintError($"File {fileName} is corrupted and cannot be loaded!");
                }
            }
        ...
        private void ChatCommandSpawnPointAdd(BasePlayer player, string command, string[] args)
        {
            ...
            _customSpawnPoints[name] = spawnPoint;
            ControllerSpawnPoint controller = new GameObject().AddComponent<ControllerSpawnPoint>();
